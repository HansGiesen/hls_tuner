...
shopt -s dotglob
cp -r /mnt/icgridio2/safe/giesen/HLS_tuner/1/TestApps/Rosetta/BNN/Output/0004/Synth/* .
...
# results to the output directory.  We check for a timeout here because we don't want to limit the time needed for
# issuing a job to the grid.  The /usr/bin/timeout tool changes its process group, which means that the children do not
# receive TERM signals, so we use a custom timeout script.
/mnt/icgridio2/safe/giesen/HLS_tuner/1/Scripts/Timeout.bash -t 1800 \
  /usr/bin/time -f "Runtime: %e s\nMaximum residential set size: %M KB" -o /dev/stdout \
    vivado -nolog -nojournal -mode batch -source /mnt/icgridio2/safe/giesen/HLS_tuner/1/TestApps/Rosetta/BNN/Output/0004/Impl/Impl.tcl && EXIT_CODE=0 || EXIT_CODE=$?
